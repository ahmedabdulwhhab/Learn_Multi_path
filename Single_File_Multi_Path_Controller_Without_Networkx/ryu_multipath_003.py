  ...
  #self.install_paths(      h1[0],       h1[1],      h2[0],       h2[1],   src_ip, dst_ip)
    def install_paths(self, src_switch, first_port, dst_switch, last_port, ip_src, ip_dst):
        ...
        try:
            ...
            """select mini cos"""
            y=paths_with_ports[0]
            for i in paths_with_ports:
                if(len(y) > len(i)):
                    y=i
            shortest_path_cost=y
            single_path = shortest_path_cost
            #####
            print(" shortest path cost from src switch to dst switch is ", single_path)
            ...
            for j in range(len(key_val) ):
                ...
                if(j== len(key_val)-1):
                    ...
                    #src switch flow entry
                    self.add_flow(src_sw_dp, 32768, match_ip, actions,idle=30, hard=60)
                    self.add_flow(src_sw_dp, 1, match_arp, actions,idle=30, hard=60)
                    ##############################################################
                    ...
                    self.en_clear_flow_entry=True
                    #src switch flow entry
                    self.add_flow(dst_sw_dp, 32768, match_ip, actions,idle=30, hard=60)
                    self.add_flow(dst_sw_dp, 1, match_arp, actions,idle=30, hard=60)
                    #j =j+5
